...
up = zeros([size(Vertices,1) 3]);
vp = zeros([size(Vertices,1) 3]);
for i=1:size(Faces,1)
    ...
    VertexNormals(Faces(i,1),:) = VertexNormals(Faces(i,1),:)+wfv1*N(i,:);
    VertexNormals(Faces(i,2),:) = VertexNormals(Faces(i,2),:)+wfv2*N(i,:);
    VertexNormals(Faces(i,3),:) = VertexNormals(Faces(i,3),:)+wfv3*N(i,:);
    ...
    up(Faces(i,1),:) = e2_norm(i,:);
    up(Faces(i,2),:) = e0_norm(i,:);
    up(Faces(i,3),:) = e1_norm(i,:);
end
